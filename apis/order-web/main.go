...
import (
	...
	"luke544187758/order-web/routes"
	"luke544187758/order-web/services"
	"luke544187758/order-web/settings"
	"luke544187758/order-web/utils"
	"luke544187758/order-web/utils/register/consul"
...
func main() {
	...
	zap.L().Info("all services are connecting...")
	...
	if err != nil {
		zap.L().Panic("order-web register failed", zap.Error(err))
	}
	zap.L().Info("order-web register success...")
	...
	if err := cli.DeRegister(settings.Conf.Name, settings.Conf.Host, settings.Conf.Port); err != nil {
		zap.L().Error("order-web deregister failed", zap.Error(err))
	}
	zap.L().Info("order-web deregister success...")
