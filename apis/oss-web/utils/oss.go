...
func Get_policy_token() string {
	now := time.Now().Unix()
	expire_end := now + settings.Conf.OssService.Timeout
	...
	condition = append(condition, settings.Conf.OssService.UploadDir)
	config.Conditions = append(config.Conditions, condition)

	//calucate signature
	result, err := json.Marshal(config)
	debyte := base64.StdEncoding.EncodeToString(result)
	h := hmac.New(func() hash.Hash { return sha1.New() }, []byte(settings.Conf.OssService.ApiSecret))
	io.WriteString(h, debyte)
	signedStr := base64.StdEncoding.EncodeToString(h.Sum(nil))

	var callbackParam CallbackParam
	callbackParam.CallbackUrl = settings.Conf.OssService.CallBackUrl
	...
	policyToken.AccessKeyId = settings.Conf.OssService.ApiKey
	policyToken.Host = settings.Conf.OssService.Host
	policyToken.Expire = expire_end
	policyToken.Signature = string(signedStr)
	policyToken.Directory = settings.Conf.OssService.UploadDir
